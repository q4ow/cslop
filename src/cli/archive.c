...
const char *get_archive_type_name(archive_type_t type) {
    switch (type) {
        case ARCHIVE_TYPE_ZIP:   return "ZIP";
        case ARCHIVE_TYPE_TAR:   return "TAR";
        case ARCHIVE_TYPE_GZIP:  return "GZIP";
        case ARCHIVE_TYPE_BZIP2: return "BZIP2";
        case ARCHIVE_TYPE_XZ:    return "XZ";
        case ARCHIVE_TYPE_7Z:    return "7Z";
        case ARCHIVE_TYPE_RAR:   return "RAR";
        default:                 return "Unknown";
    }
}

static char *create_extraction_dir(const char *archive_path, const struct extract_options *options) {
    char *dir;
    if (options->destination) {
        dir = strdup(options->destination);
    } else if (options->create_subdirectory) {
        char *base = basename((char *)archive_path);
...
int extract_archive(const char *filename, const struct extract_options *options) {
